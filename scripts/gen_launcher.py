#!/usr/bin/env python3
"""(Re)generate AgentForge.bat as GBK+CRLF — cmd.exe requires the ANSI/OEM
codepage for reliable batch parsing, while the workflow YAMLs are UTF-8, so
the launcher sets PYTHONUTF8=1 for the Python side and keeps the console at
cp936 via PYTHONIOENCODING."""
...
data = BAT.replace("\n", "\r\n").encode("gbk")  # strict — raises on unencodable chars
open("AgentForge.bat", "wb").write(data)
print(f"AgentForge.bat regenerated: {len(data)} bytes, GBK+CRLF, no BOM: {data[:3] != b'\xef\xbb\xbf'}")
